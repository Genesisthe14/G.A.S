...
//Class implementing a Dictionary which can be serialized by unity
...
#region IDictionary<TKey, TValue>
    ...
    public bool Remove (TKey key) {
        if (KeyPositions.TryGetValue(key, out uint index)) {
            ...
            //first remove object from list at given index
            list.RemoveAt( (int) index );

            //then adjust key positions to match values again
            var numEntries = list.Count;
